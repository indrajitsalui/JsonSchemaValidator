      ...
      <v-row no-gutters>
        ...
        <v-col cols="12" md="6" class="pl-5">
          <v-card class="jsonInputCard" outlined tile>
            ...
            <CodeViewer :code="jsonStringifiedOutput" />
          </v-card>
        </v-col>
      </v-row>
      <v-row>
    ...
    "jsonInput.value": {
      handler(jsonInput) {
        if (this.validateJsonInput(jsonInput)) {
          // var myJSONString = JSON.stringify(JSON.parse(jsonInput));
          this.jsonStringifiedOutput.value = jsonInput
            .replace(/\n/g, "\\n")
            .replace(/'/g, "\\'")
            .replace(/"/g, '\\"')
            .replace(/&/g, "\\&")
            ...
            .replace(/\t/g, "\\t")
            .replace(/\f/g, "\\f");
            console.log(this.jsonStringifiedOutput.value)
        }
      },
      immediate: true,
    },
